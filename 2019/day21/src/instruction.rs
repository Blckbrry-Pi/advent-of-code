...
opcode_def!(
    pub enum Instruction {
        ...
        In [3] { out to } => <machine> { to(machine.input()) },
        Out [4] { in val } => <machine> { machine.output(val) },
        JumpIfTrue [5] { in cond, in addr } => custom<machine, _data> {
            if cond != 0 {
        ...
        JumpIfFalse [6] { in cond, in addr } => custom<machine, _data> {
            if cond == 0 {
                machine.pc = addr as usize;
            } else {
                machine.pc += 3;
        ...
        Lt [7] { in l, in r, out to } => <>{ to((l < r) as isize) },
        Eq [8] { in l, in r, out to } => <>{ to((l == r) as isize) },
        AdjRel [9] { in adj } => <machine>{
            machine.offset += adj;
        },

        Halt [99] {} => <machine>{ machine.halt = true; },
    }
);
